...
export default function RecipeCard({ recipe, saved = false, subtitle, href, locale = "en" }) {
  const imageClass = recipe.image || recipe.cardClass;
  const displayTime = recipe.time || recipe.prep_time || recipe.prepTime;

  return (
    <article className="recipe-card">
      <Link href={href || localizeHref(locale, `/recipes/${recipe.slug}`)} className="recipe-card-link">
        <div className="recipe-card-img">
          <div className={`recipe-card-img-inner ${imageClass}`} />
