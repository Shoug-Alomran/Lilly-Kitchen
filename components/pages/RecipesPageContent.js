...
export default function RecipesPageContent({ searchParams, locale = "en" }) {
  const dictionary = getDictionary(locale);
  const labels = dictionary.recipes;
  const activeCategory = searchParams?.category || "All";
  const activeCollection = searchParams?.collection || "";
  const searchQuery = searchParams?.search || "";
  const categories = getCategories({ locale, includeAll: true });
  const recipes = getRecipeCards({
    locale,
    category: activeCollection ? "All" : activeCategory,
    ...
    <main className="page-shell">
      ...
      <section className="page-shell__content">
        ...
        <div className="filter-bar">
          {categories.map((category) => (
            <Link
              ...
              href={
                category.name === "All"
                  ? localizeHref(locale, "/recipes")
                  : localizeHref(locale, `/recipes?category=${encodeURIComponent(category.name)}`)
              }
        ...
        <div className="results-copy">
          ...
          {activeCollection
            ? ` ${labels.in} ${translateCollection(activeCollection, locale)}`
            : activeCategory !== "All"
              ? ` ${labels.for} ${translateCategory(activeCategory, locale)}`
              : ""}
